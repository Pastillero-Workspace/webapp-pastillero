...
<script type="text/javascript" language="javascript" class="init">
		...
		window.editar = function editar(){
			...
			$('#example tbody').on('click','button',function(){
				...
				//console.log(key ,value);
					if(key == 0)$('#txtCantidad').val(value);
	...
	$("#txtCantidad").keypress(function(e){
		if(e.which == 13){
			...
			if(!cantidad.startsWith("0")){
				...
				if(cantidad < 1000){
					if(cantidad > 10){
						var confirmar = confirm("Esta seguro que quiere agregar el numero de piezas a la recepcion, \n" +
					...
					alert("No puede hacer una recepcion con esa cantidad para un mismo producto");
					$('#txtCantidad').select();
				}
			}else{
				alert("Verifique que la cantidad sea valida!");
				$('#txtCantidad').select();
			}
		}
	});
	...
	$('#btnGuardar').on('click ',function(e){
		...
		var iva = $('#lblIva').val();
		var ieps1 = $('#lblIeps1').val();
		var ieps2 = $('#lblIeps2').val();
		var total = $('#lblTotal').val();
		//var filas = t.length;
		var table = $("#example").tableToJSON({ignoreColumns: [2]});
		...
		if(fecha.trim()!=""&&factura.trim()!=""&&usuario.trim()!=""&&proveedor.trim()!=""&&desc1.trim()!=""&&desc2.trim()!=""&&folioE.trim()!=""&&subtotal.trim()!=""){
			$.ajax({
				...
				$.unblockUI();
				alert("Recepcion guardada con éxito.");
